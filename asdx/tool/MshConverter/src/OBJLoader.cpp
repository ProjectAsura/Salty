...
void OBJMESH::WriteDirect( FILE* pFile )
{
    ...
    //dataHeader.numBones     = 0;
    dataHeader.vertexStructureSize   = MSH_VERTEX_STRUCT_SIZE;
    dataHeader.indexStructureSize    = MSH_INDEX_STRUCT_SIZE;
    dataHeader.materialStructureSize = MSH_MATERIAL_STRUCT_SIZE;
    dataHeader.subsetStructureSize   = MSH_SUBSET_STRUCT_SIZE;
    //dataHeader.boneStructureSize     = MSH_BONE_STRUCT_SIZE;

    // ファイルヘッダーを設定.
    MSH_FILE_HEADER fileHeader;
    memcpy( &fileHeader.magic, MSH_MAGIC, sizeof( unsigned char ) * 4 );
    fileHeader.version        = MSH_VERSION;
    fileHeader.dataHeaderSize = MSH_DATA_HEADER_SIZE;
    ...
    // ヘッダーを書き込み.
    fwrite( &fileHeader, sizeof( MSH_FILE_HEADER ), 1, pFile );

    // 頂点データを書き込み.
    for( size_t i=0; i<m_NumVertices; ++i )
    {
        MSH_VERTEX vertex;
        ZeroMemory( &vertex, sizeof( MSH_VERTEX ) );
        ...
        // 接ベクトル.
        vertex.tangent.x  = m_Vertices[ i ].tangent.x;
        vertex.tangent.y  = m_Vertices[ i ].tangent.y;
        vertex.tangent.z  = m_Vertices[ i ].tangent.z;

        // テクスチャ座標.
        vertex.texcoord.x = m_Vertices[ i ].texcoord.x;
        vertex.texcoord.y = m_Vertices[ i ].texcoord.y;
        ...
        fwrite( &vertex, sizeof( MSH_VERTEX ), 1, pFile );
    }
    ...
    {
        MSH_MATERIAL material;
        ZeroMemory( &material, sizeof( MSH_MATERIAL ) );

        // 環境色.
        material.ambient.x = m_Materials[ i ].ambient.x;
        material.ambient.y = m_Materials[ i ].ambient.y;
        material.ambient.z = m_Materials[ i ].ambient.z;
        ...
        // 透過度.
        material.alpha     = m_Materials[ i ].alpha;

        // テクスチャマップ名の拡張子を.mapに変更.
        std::string temp;
        std::string::size_type index;

        temp = m_Materials[ i ].ambientMapName;
        ...
        strcpy( material.ambientMap,      temp.c_str() );


        temp = m_Materials[ i ].diffuseMapName;
        index = temp.find_last_of( "." );
        if ( index != std::string::npos )
        {
            temp = temp.substr( 0, index );
        ...
        strcpy( material.diffuseMap,      temp.c_str() );

        
        temp = m_Materials[ i ].specularMapName;
        index = temp.find_last_of( "." );
        if ( index != std::string::npos )
        {
            temp = temp.substr( 0, index );
        ...
        strcpy( material.specularMap,     temp.c_str() );

        
        temp = m_Materials[ i ].bumpMapName;
        index = temp.find_last_of( "." );
        if ( index != std::string::npos )
        {
            temp = temp.substr( 0, index );
        ...
        strcpy( material.bumpMap,         temp.c_str() );

        
        temp = m_Materials[ i ].displacementMapName;
        index = temp.find_last_of( "." );
        if ( index != std::string::npos )
        {
            temp = temp.substr( 0, index );
        ...
        strcpy( material.displacementMap, temp.c_str() );
        ...
        fwrite( &material, sizeof( MSH_MATERIAL ), 1, pFile );
    }
    ...
    {
        MSH_SUBSET subset;
        ZeroMemory( &subset, sizeof( MSH_SUBSET ) );
        ...
        fwrite( &subset, sizeof( MSH_SUBSET ), 1, pFile );
    }
}
